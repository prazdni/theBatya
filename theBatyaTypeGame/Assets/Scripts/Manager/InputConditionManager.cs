...
public class InputConditionManager : MonoBehaviour
{
    ...
    private void PrepareText(int index)
    {
        if (index == 0)
        {
            _phraseToCheck = StaticHelper.DRINK;
        }
        if (index == 1)
        {
            _phraseToCheck = StaticHelper.SMOKE;
        }
        if (index == 2)
        {
            _phraseToCheck = StaticHelper.USE;
        }
    ...
    private void SetPhrase()
    {
        ...
        if (_level == 0)
        {
            ...
            var alpha = StaticHelper.ALPHABET;
